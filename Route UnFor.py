...
# Função de geocodificação (endereço -> coordenadas)
def forward_geocode(endereco):
    ...
    querystring = {"address": endereco, "language": "pt"}
    headers = {
...
# Campo para origem de partida (sempre informado pelo usuário)
...
if uploaded_file and all(placas) and endereco_origem:
    ...
    if not origem_coords:
        st.error("❌ Não foi possível localizar o endereço de origem. Tente simplificar (ex.: 'Av. Santos Dumont 1470, Fortaleza, CE').")
        st.stop()
