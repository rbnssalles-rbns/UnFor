...
if arquivo:
    ...
    # Filtro de rotas (abre já em "Todas")
    ...
    for i, ((cliente_id, rota_nome), grupo) in enumerate(df.groupby(["Cliente_ID","Cliente"])):
        ...
        # Camada de caminho com id único
        path_layer = pdk.Layer(
            "PathLayer",
            data=[{"id": f"{cliente_id}_{rota_nome}", "path":caminho,"name":f"Rota {rota_nome} ({cliente_id})"}],
            get_path="path",
            get_width=4,
            get_color=cor_rota,
            width_min_pixels=2
        ...
        # Ícones com URL distinta por rota e IDs únicos
        icon_data = [
