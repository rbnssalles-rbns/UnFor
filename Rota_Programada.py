...
# Chaves de API
# -------------------------------
ORS_KEY = "SUA_CHAVE_ORS_AQUI"
OPENCAGE_KEY = "SUA_CHAVE_OPENCAGE_AQUI"
...
def gerar_rota_real(origem_lat, origem_lon, pontos, otimizar=False):
    ...
    try:
        if otimizar:
            # Fluxo otimizado (TSP)
            jobs = [{"id": idx, "location": p} for idx, p in enumerate(coords[1:], start=1)]
            vehicles = [{
                "id": 1,
                "start": coords[0],
                "profile": "driving-car",
            ...
            caminho = convert.decode_polyline(rota['routes'][0]['geometry'])['coordinates']
            summary = rota['routes'][0]['summary']
            steps = rota['routes'][0]['steps']

            # Converter steps em segments
            segments = [{"distance": s["distance"], "duration": s["duration"]}
                        for s in steps if "distance" in s and "duration" in s]

        else:
            # Fluxo normal (ordem do Excel)
