...
if arquivo:
    ...
    # Mapeamento de cor para clientes (RGB)
    color_map = {
        "Azul": [0, 0, 255],
        "Verde": [0, 128, 0],
        "Laranja": [255, 165, 0],
    ...
    client_color_rgb = color_map.get(color_option, [0, 0, 255])  # default azul
    ...
    for i, ((cliente_id, rota_nome), grupo) in enumerate(df.groupby(["Cliente_ID","Cliente"])):
        ...
        # Camada dos ícones (CD + clientes)
        icon_data = [
            {"lat": cd_lat, "lon": cd_lon, "name": "Centro de Distribuição", "icon": "cd"}
        ] + [
            {"lat": p["lat"], "lon": p["lon"], "name": p["name"], "icon": "cliente"}
            for p in pontos
        ]
        ...
        # Adiciona cor dinâmica aos dados
        for d in icon_data:
            if d["icon"] == "cliente":
                d["color"] = client_color_rgb
            else:
                d["color"] = [255, 0, 0]  # vermelho para CD
