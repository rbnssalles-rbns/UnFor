...
def gerar_rota_real(origem_lat, origem_lon, pontos, otimizar=False):
    ...
    pontos = pontos[:50]  # limitar a 50 pontos
...
if arquivo:
    ...
    # Rotas por veículo
    # -------------------------------
    st.subheader("🗺️ Rotas por veículo")
    cores = [[255,0,0],[0,128,255],[0,200,0],[255,165,0],[128,0,128]]  # paleta de cores

    layers = []
    view_state = pdk.ViewState(latitude=origem_lat, longitude=origem_lon, zoom=11)

    for i, (cliente_id, grupo) in enumerate(df.groupby("Cliente_ID")):
        pontos = [
        ...
        st.markdown(f"**Veículo {cliente_id}** — Distância total: {dist_total:.2f} km | Tempo total: {tempo_total:.1f} min")
        ...
        path_data = [{"path": caminho, "name": f"Rota {cliente_id}"}]
