...
if arquivo:
    ...
    for i, ((cliente_id, rota_nome), grupo) in enumerate(df.groupby(["Cliente_ID","Cliente"])):
        ...
        # Dados dos ícones (interior branco e borda colorida)
        icon_data_inner = [
            {"lat": cd_lat, "lon": cd_lon, "name": "Centro de Distribuição", "icon": "cd", "color": [255,255,255]}
        ] + [
            {"lat": p["lat"], "lon": p["lon"], "name": p["name"], "icon": "cliente", "color": [255,255,255]}
            for p in pontos
        ]
        ...
        # Interior branco
        icon_layer_inner = pdk.Layer(
            "IconLayer",
            data=icon_data_inner,
            get_icon="icon",
            get_size=icon_size,
            ...
            icon_mapping={
                "cd": {"x":0,"y":0,"width":128,"height":128,"anchorY":128,"mask":True,"url":cd_marker_url},
                "cliente": {"x":0,"y":0,"width":128,"height":128,"anchorY":128,"mask":True,"url":client_marker_url}
            }
        )

        # Borda colorida (maior)
        icon_layer_border = pdk.Layer(
            "IconLayer",
            data=icon_data_border,
            get_icon="icon",
            get_size=icon_size,
            size_scale=size_scale * 1.3,  # maior para parecer borda
            get_position='[lon, lat]',
            get_color="color",
            pickable=True,
            icon_atlas="https://raw.githubusercontent.com/visgl/deck.gl-data/master/website/icon-atlas.png",
        ...
        layers.extend([path_layer, icon_layer_border, icon_layer_inner])
