...
if arquivo:
    ...
    for i, ((cliente_id, rota_nome), grupo) in enumerate(df.groupby(["Cliente_ID","Cliente"])):
        ...
        pontos = [
            {"lat": r["Latitude"], "lon": r["Longitude"], "name": r["Endereco"], "cliente_id": r["Cliente_ID"], "rota": r["Cliente"]}
        ...
        if resumo:
            ...
            if isinstance(segmentos, list):
                ...
                for j, seg in enumerate(segmentos):
                    ...
                    endereco_destino = pontos_ordenados[j]["name"]
    ...
    # Tabelas organizadas abaixo do mapa com download
    if tabela_resumo:
        ...
        st.download_button(
            label="⬇️ Baixar Resumo das Rotas (Excel)",
    ...
    if tabela_detalhada:
        ...
        df_detalhada = pd.DataFrame(tabela_detalhada)
        st.dataframe(df_detalhada)

        buffer = io.BytesIO()
        df_detalhada.to_excel(buffer, index=False)
        st.download_button(
            label="⬇️ Baixar Detalhamento De/Para (Excel)",
            data=buffer.getvalue(),
            file_name="detalhamento_rotas.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
