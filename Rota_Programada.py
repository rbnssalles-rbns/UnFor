...
from opencage.geocoder import Geocoder
...
def geocode_opencage(endereco):
    geolocator = Geocoder(api_key=API_KEY_OPENCAGE)
    try:
...
cd_lat_manual = st.sidebar.text_input("Latitude do CD", value=f"{cd_lat:.6f}" if cd_lat else "")
cd_lon_manual = st.sidebar.text_input("Longitude do CD", value=f"{cd_lon:.6f}" if cd_lon else "")
