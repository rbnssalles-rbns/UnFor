...
# Função para obter rota real pelas ruas com ORS
...
def nearest_neighbor(coords):
    rota = [coords[0]]  # começa no primeiro ponto
    restantes = coords[1:]
    while restantes:
        ultimo = rota[-1]
        prox = min(restantes, key=lambda c: ((c[0]-ultimo[0])**2 + (c[1]-ultimo[1])**2))
        rota.append(prox)
...
if uploaded_file:
    ...
    metodo = st.radio("📌 Escolha o método de rota:", 
                      ["Seguir ordem do arquivo", "Otimizar pela proximidade"])
    ...
    for i, (placa, grupo) in enumerate(df.groupby("Cliente_ID")):
        ...
        if metodo == "Otimizar pela proximidade" and len(coords) > 1:
            coords = nearest_neighbor(coords)

        # Obter trajeto real pelas ruas
