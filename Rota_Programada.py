...
if arquivo:
    ...
    for i, ((cliente_id, rota_nome), grupo) in enumerate(df.groupby(["Cliente_ID","Cliente"])):
        ...
        # Dados dos pontos (clientes + CD)
        pontos_data = pontos + [{"lat": cd_lat, "lon": cd_lon, "name": "Centro de Distribuição"}]
        ...
        border_layer = pdk.Layer(
            ...
            get_radius=150,  # raio maior
            get_fill_color=lambda d: cor_rota if d["name"] != "Centro de Distribuição" else [255,0,0],
            pickable=True
        )

        # Interior branco
        inner_layer = pdk.Layer(
            "ScatterplotLayer",
            data=pontos_data,
            get_position='[lon, lat]',
            get_radius=100,  # raio menor
            get_fill_color=[255,255,255],  # branco
            pickable=True
        )

        layers.extend([path_layer, border_layer, inner_layer])
