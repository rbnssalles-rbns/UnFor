...
def geocode_endereco_com_fallback(endereco, lat_default=-3.73, lon_default=-38.54):
    try:
        ...
        return lat_default, lon_default, "Estimado"
    # Função para gerar rota real via ORS
def gerar_rota_real(pontos_sequencia):
    if CLIENT_ORS is None or not pontos_sequencia:
        return [], None

    # Montar lista de coordenadas [lon, lat]
    coords = [[p["lon"], p["lat"]] for p in pontos_sequencia]
    ...
    try:
        ...
        # Extrair geometria e resumo
        caminho = rota['features'][0]['geometry']['coordinates']
        resumo = rota['features'][0]['properties']['summary']

        # Converter para lista de dicts {lon, lat}
        rota_caminho = [{"lon": lon, "lat": lat} for lon, lat in caminho]

        return rota_caminho, resumo

    except Exception as e:
        st.warning(f"Erro ao gerar rota real: {e}")
        return [], None


# BLOCO 2 — Importação da base única com geocodificação
...
if arquivo_base:
    ...
    # Conferir se a coluna ABERTURA existe
    if "ABERTURA" in df_base.columns:
        # Converter coluna ABERTURA (data e hora) para datetime
        df_base["ABERTURA"] = pd.to_datetime(df_base["ABERTURA"], errors="coerce", dayfirst=True)
        # Criar coluna apenas com a data (datetime.date)
    ...
    # Geocodificação com fallback (gera LAT/LON e status)
...
# BLOCO 3 — Filtros, trechos origem→destino e rota real
...
if not df_filtrado.empty:
    st.subheader("🗺️ Rotas das corridas")

    # Construir lista de trechos origem→destino
    trechos = []
    for _, r in df_filtrado.dropna(subset=["LAT_ORIGEM","LON_ORIGEM","LAT_DESTINO","LON_DESTINO"]).iterrows():
        trechos.append({
            "origem": {"lat": float(r["LAT_ORIGEM"]), "lon": float(r["LON_ORIGEM"]), "name": f"Origem - {r['SOLICITANTE']}"},
            "destino": {"lat": float(r["LAT_DESTINO"]), "lon": float(r["LON_DESTINO"]), "name": f"Destino - {r['SOLICITANTE']}"}
        })

    # Configuração de cor dos marcadores
    ...
    # Calcular rotas reais para cada trecho
    todas_rotas = []
    total_dist, total_dur = 0, 0
    for trecho in trechos:
        coords = [ [trecho["origem"]["lon"], trecho["origem"]["lat"]],
                   [trecho["destino"]["lon"], trecho["destino"]["lat"]] ]
        rota = CLIENT_ORS.directions(coordinates=coords, profile='driving-car', format='geojson')
        caminho = rota['features'][0]['geometry']['coordinates']
        resumo = rota['features'][0]['properties']['summary']
        todas_rotas.append(caminho)
        total_dist += resumo["distance"]
        total_dur += resumo["duration"]

    # Camadas de pontos
    pontos_plot = [t["origem"] for t in trechos] + [t["destino"] for t in trechos]
    icon_layer = pdk.Layer("ScatterplotLayer", data=pontos_plot,
                           get_position='[lon, lat]', get_radius=60,
                           get_fill_color=cor_marcador, pickable=True)

    # Camadas de rotas
    path_layers = []
    for caminho in todas_rotas:
        path_layers.append(pdk.Layer(
            "PathLayer",
            data=[{"path": [[lon, lat] for lon, lat in caminho]}],
            get_path="path", get_width=4, get_color=[0, 128, 255], width_min_pixels=2
        ))

    # Estado inicial da visualização
    view_state = pdk.ViewState(latitude=-3.831753, longitude=-38.613147, zoom=11)

    # Renderizar mapa com marcadores + todas as rotas
    st.pydeck_chart(pdk.Deck(layers=[icon_layer] + path_layers,
