...
if arquivo_base:
    ...
    col_obrig = ["ABERTURA","SOLICITANTE","ORIGEM","DESTINO EFETIVO",
    ...
    st.success(f"{len(df_base)} registros carregados e geocodificados.")
# BLOCO 3 — Filtros e simulação
dia_escolhido = st.sidebar.date_input("Escolha a data da solicitação")
df_filtrado = pd.DataFrame()
if dia_escolhido:
    df_filtrado = df_base[df_base["ABERTURA"].dt.date == dia_escolhido].copy()

if not df_filtrado.empty:
    st.subheader("🗺️ Mapa das solicitações")
    pontos = df_filtrado.dropna(subset=["LAT_DESTINO","LON_DESTINO"]).apply(
        lambda r: {"lat": r["LAT_DESTINO"], "lon": r["LON_DESTINO"], "name": str(r["SOLICITANTE"])}, axis=1
    ).tolist()

    layer = pdk.Layer("ScatterplotLayer", data=pontos,
                      get_position='[lon, lat]', get_radius=60,
                      get_fill_color=[0,122,255,160], pickable=True)
    view_state = pdk.ViewState(latitude=np.mean([p["lat"] for p in pontos]),
                               longitude=np.mean([p["lon"] for p in pontos]), zoom=11)
