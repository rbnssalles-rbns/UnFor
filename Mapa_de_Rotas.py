...
# BLOCO 3 — Filtros e simulação (ajustado para tipos compatíveis)

dia_escolhido = st.sidebar.date_input("Escolha a data da solicitação")

df_filtrado = pd.DataFrame()
if dia_escolhido:
    # Converter ABERTURA_DATA para datetime.date antes da comparação
    df_filtrado = df_base[df_base["ABERTURA_DATA"] == pd.to_datetime(dia_escolhido).date()].copy()
