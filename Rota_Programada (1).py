...
if arquivo:
    ...
    # Renderização do mapa com fit_bounds
    lat_min, lat_max = min([cd_lat] + df["Latitude"].dropna().tolist()), max([cd_lat] + df["Latitude"].dropna().tolist())
    lon_min, lon_max = min([cd_lon] + df["Longitude"].dropna().tolist()), max([cd_lon] + df["Longitude"].dropna().tolist())
    bounds = [[lon_min, lat_min], [lon_max, lat_max]]

    deck = pdk.Deck(layers=layers, initial_view_state=pdk.ViewState(latitude=cd_lat, longitude=cd_lon, zoom=11), tooltip={"text": "{name}"})
    deck.fit_bounds(bounds)
