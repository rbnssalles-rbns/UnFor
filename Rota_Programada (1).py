...
# -------------------------------
# Processamento do Excel revisado e integrado
# -------------------------------
if arquivo:
    ...
    for i, ((cliente_id, rota_nome), grupo) in enumerate(df.groupby(["Cliente_ID","Cliente"])):
        ...
        # Aplicar vizinho mais próximo corretamente
        pontos_ordenados = vizinho_mais_proximo(cd_lat, cd_lon, pontos) if opcao=="Vizinho mais próximo" else pontos

        # Usar função com limite de pontos
        caminho, resumo, segmentos = gerar_rota_com_limite(cd_lat, cd_lon, pontos_ordenados, debug=False)

        if resumo:
            tabela_resumo.append({
                "Veículo": cliente_id,
                "Rota": rota_nome,
                "Distância Total (km)": f"{resumo.get('distance_km',0):.2f}",
                "Tempo Total (min)": f"{resumo.get('duration_min',0):.2f}"
            })
            ...
            for j, seg in enumerate(segmentos):
                ...
                tabela_detalhada.append({
                    ...
                    "De": origem,
                    "Para": destino,
                    "Distância (km)": f"{seg['distance_km']:.2f}",
                    "Tempo (min)": f"{seg['duration_min']:.2f}"
                })
                origem = destino
        ...
        # Camada da rota com id único
        path_data = [{"id": f"{cliente_id}_{rota_nome}", "path": caminho, "name": f"Rota {rota_nome} ({cliente_id})"}]
        ...
        # Dados dos ícones com IDs únicos
        icon_data = [
            {"id": f"CD_{cliente_id}_{rota_nome}", "lat": cd_lat, "lon": cd_lon, "name": "Centro de Distribuição", "icon": "cd"}
        ] + [
            {"id": f"{cliente_id}_{rota_nome}_{j}", "lat": p["lat"], "lon": p["lon"], "name": p["name"], "icon": "cliente"}
        ...
        icon_layer = pdk.Layer(
            ...
            get_position=["lon","lat"],  # corrigido
            pickable=True,
            icon_atlas="https://raw.githubusercontent.com/visgl/deck.gl-data/master/website/icon-atlas.png",
            icon_mapping={
                "cd": {"url": cd_marker_url, "width": 128, "height": 128, "anchorY": 128},
    ...
    # Renderização do mapa acumulando todas as rotas
