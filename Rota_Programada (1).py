...
# -------------------------------
# Função para gerar rota real com ORS (CORRIGIDA)
# -------------------------------
def gerar_rota_real(cd_lat, cd_lon, pontos):
    if CLIENT_ORS is None or not pontos:
        return [], None, []

    # CD no início e no fim da rota
    coords = [[cd_lon, cd_lat]] + [[p["lon"], p["lat"]] for p in pontos] + [[cd_lon, cd_lat]]
    ...
    try:
        ...
        feature = rota['features'][0]
        props = feature['properties']

        # Caminho
        caminho = feature['geometry']['coordinates']
        ...
        # ✅ Segmentos corrigidos (sem object)
        segmentos = []
        for i, seg in enumerate(props.get("segments", [])):
            segmentos.append({
                "ordem": i + 1,
                "distance_km": round(seg.get("distance", 0) / 1000, 3),
                "duration_min": round(seg.get("duration", 0) / 60, 2),
                "inicio": seg.get("start_index"),
                "fim": seg.get("end_index")
            })

        return caminho, resumo, segmentos

    except Exception as e:
        st.error(f"Erro ao gerar rota ORS: {e}")
        return [], None, []


# -------------------------------
# Processamento do Excel corrigido
# -------------------------------
if arquivo:
    ...
    # Filtro de rotas
    rotas_disponiveis = df["Cliente"].dropna().unique().tolist()
    rota_selecionada = st.sidebar.selectbox(
        "Selecione uma rota para visualizar",
        ["Todas"] + rotas_disponiveis
    )

    # Escolha de simulação
    opcao = st.sidebar.radio(
        "Escolha a simulação:",
        ["Ordem original", "Vizinho mais próximo"]
    )
    ...
    for i, ((cliente_id, rota_nome), grupo) in enumerate(df.groupby(["Cliente_ID", "Cliente"])):
        ...
        # Ordenação
        pontos_ordenados = (
            vizinho_mais_proximo(cd_lat, cd_lon, pontos)
        ...
        caminho = [[p[0], p[1]] for p in caminho]

        # Preencher tabelas
        if resumo:
            resumo["Rota"] = rota_nome
            resumo["Cliente_ID"] = cliente_id
            tabela_resumo.append(resumo)

        if segmentos:
            for seg in segmentos:
                seg["Rota"] = rota_nome
                seg["Cliente_ID"] = cliente_id
            tabela_detalhada.extend(segmentos)

        cor_rota = cores[i % len(cores)]
        client_marker_url = marker_urls_por_rota[i % len(marker_urls_por_rota)]

        # Camada da rota
        path_data = [{
            "id": f"{cliente_id}_{rota_nome}",
            "path": caminho,
        ...
        icon_data = [
            {
                "id": f"CD_{cliente_id}_{rota_nome}",
                "lat": cd_lat,
                "lon": cd_lon,
            ...
            {
                "id": f"{cliente_id}_{rota_nome}_{j}",
                "lat": p["lat"],
                "lon": p["lon"],
                "name": p["name"],
                "icon": "cliente"
            }
            for j, p in enumerate(pontos_ordenados)
        ]
