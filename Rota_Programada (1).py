...
if arquivo:
    ...
    for i, ((cliente_id, rota_nome), grupo) in enumerate(df.groupby(["Cliente_ID","Cliente"])):
        # Se o usuário escolheu uma rota específica, só mostra ela
        ...
        if resumo:
            ...
            tabela_resumo.append({
                ...
                "Tempo Total": f"{horas}h {minutos}min"
            })
        ...
        path_layer = pdk.Layer(
            "PathLayer",
            data=[{"path":caminho,"name":f"Rota {rota_nome} ({cliente_id})"}],
            get_path="path",
            get_width=4,
            get_color=cor_rota,
            width_min_pixels=2
    ...
    # Renderização do mapa com todas ou apenas a rota escolhida
