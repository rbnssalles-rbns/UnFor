...
if arquivo:
    ...
    # Loop por rota (agrupando apenas por Cliente)
    for i, (rota_nome, grupo) in enumerate(df.groupby("Cliente")):
        if rota_selecionada != "Todas" and rota_nome != rota_selecionada:
            continue

        pontos = [
            {"lat": r["Latitude"], "lon": r["Longitude"], "name": f"Rota: {r['Cliente']} | Endereço: {r['Endereco']}"}
        ...
        # Camada da rota
        path_data = [{"path": caminho, "name": f"Rota {rota_nome}"}]
        ...
        # acumula todas as rotas
        layers.append(path_layer)
        layers.append(icon_layer)

    # Renderização do mapa com todas as rotas acumuladas
