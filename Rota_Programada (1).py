...
if cd_lat is not None and cd_lon is not None:
    cd_coord_manual = st.sidebar.text_input(
        ...
        value=f"{cd_lat:.6f}, {cd_lon:.6f}"
    )
else:
    cd_coord_manual = st.sidebar.text_input("Coordenadas do CD (lat, lon)", value="")
...
if arquivo:
    ...
    # Filtro de rotas
    rotas_disponiveis = df["Cliente"].unique().tolist()  # ou df["Rota"] se existir
    ...
    # Loop por rota (ajuste no groupby)
    for i, (rota_nome, grupo) in enumerate(df.groupby("Cliente")):  # ajuste aqui se houver coluna "Rota"
        ...
        # Debug: validar clientes incluídos
        st.write("Rota:", rota_nome, "Clientes incluídos:", pontos)
        ...
        if resumo:
            ...
            tabela_resumo.append({
                "Veículo": grupo["Cliente_ID"].iloc[0],
                "Rota": rota_nome,
        ...
        # Camada da rota (mantém [lon, lat] do ORS)
        path_data = [{
            "path": caminho,
            "name": f"Rota {rota_nome} ({grupo['Cliente_ID'].iloc[0]})"
        }]
        ...
        # Adiciona camadas desta rota
        layers.extend([path_layer, icon_layer])

    # Debug: validar todas as camadas acumuladas
    st.write("Camadas acumuladas:", [layer.data for layer in layers])

    # Renderização do mapa (fora do loop, com todas as camadas acumuladas)
    st.pydeck_chart(
        pdk.Deck(
            layers=layers,
